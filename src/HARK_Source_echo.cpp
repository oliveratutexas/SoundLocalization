...
void harkSourceCallback(const hark_msgs::HarkSource::ConstPtr& msg) {
	...
	for(int num = 0; num < msg->exist_src_num; num++) {
		...
		ROS_INFO("Found sound source at (%f,%f,%f) with power %f",
				src.position.x, src.position.y, src.position.z, msg->src[num].power);
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "HARK Source echo");
	ros::NodeHandle n;

	// Advertise to marker topic
	geom_pose_pu = n.advertise<geometry_msgs::Pose>("HarkSourcePosePosition", 1000);
